...
export class UserService {
    ...
    async setProfileUsernameByusername(userId: number, new_username: string): Promise<boolean> {
        ...
        if (user.username === new_username) {
            throw new HttpException('Cannot set same username', HttpStatus.BAD_REQUEST);
        }
    ...
    async acceptFriendRequestByUsername(username: string, friendUsername: string) {
        try {
            const user = await this.prisma.user.findUnique({
                where: { username: username },
            });
    ...
    async rejectFriendRequestByUsername(userId: number, friendUsername: string) {
    ...
    async getAvatarUrlById(id: number) {
        ...
        if (!user.avatarUrl) throw new NotFoundException(`User id ${id} has no avatar`);
        if (user.avatarUrl.startsWith('http')) return user.avatarUrl;
        if (user.avatarUrl.startsWith('/')) return process.env.BACKEND_URL + user.avatarUrl;
